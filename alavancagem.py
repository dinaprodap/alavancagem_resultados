...
with tab1:
    ...
    with col1:
        ...
        with cons_col2:
            ...
            st.metric("Consumo (%PV) - Molécula 2", f"{consumo_pv_mol2:.4f}")
        with cons_col3:
        ...
        with pv_final_col1:
            pv_final = st.number_input("PV Final (Kg/Cab)", min_value=0, value=560, step=1)
            pv_final_arroba = (pv_final * rendimento_carcaca/100) / 15
            st.metric("PV Final (@/Cab)", f"{pv_final_arroba:.2f}")
            
        with pv_final_col2:
            pv_final_mol2 = pv_inicial + (gmd * 1.077 * 1.02 * dias)
        ...
        with pv_final_col3:
            pv_final_mol3 = pv_inicial + (gmd * 1.118 * 1.02 * dias)
